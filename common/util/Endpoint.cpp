...
QString Endpoint::toString() const {
  ...
  if (addr_converted.protocol() == QAbstractSocket::IPv6Protocol)
    endpoint_template = QStringLiteral("[%1]:%2");
...
Endpoint Endpoint::fromSockaddr(const sockaddr& sa) {
  quint16 port_be = 0;
  switch (sa.sa_family) {
    case AF_INET: port_be = reinterpret_cast<const sockaddr_in*>(&sa)->sin_port; break;
    case AF_INET6: port_be = reinterpret_cast<const sockaddr_in6*>(&sa)->sin6_port; break;
    default: throw InvalidAddressFamily();
  }
...
std::tuple<sockaddr_storage, size_t> Endpoint::toSockaddr() const {
  ...
  switch (addr.protocol()) {
    ...
    default: return std::make_tuple(sa, sizeof(sockaddr_storage));
  }
}
