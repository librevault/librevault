...
class AvailabilityMap {
 public:
  ...
  static bool slice_superset(
      block_type subset, block_type superset, block_type& block_left, block_type& block_right) {
    ...
    if (subset.first < superset.first || subset_last_byte_offset > superset_last_byte_offset)
      return false;
  ...
  std::pair<const_iterator, bool> insert(block_type block) {
    if (block.first >= size_original_ || block.first + block.second > size_original_ ||
        available_map_.empty())
      return {end(), false};

    auto space_it =
        available_map_.upper_bound(block.first);  // This returns the block, following the inserted.
