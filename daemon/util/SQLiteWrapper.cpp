...
SQLValue::SQLValue(const std::string& text_val)
    : value_type(ValueType::TEXT), text_val(text_val.data()), size(text_val.size()) {}
SQLValue::SQLValue(const char* text_ptr, uint64_t text_size)
    : value_type(ValueType::TEXT), text_val(text_ptr), size(text_size) {}

SQLValue::SQLValue(const std::vector<uint8_t>& blob_val)
    : value_type(ValueType::BLOB), blob_val(blob_val.data()), size(blob_val.size()) {}
SQLValue::SQLValue(const uint8_t* blob_ptr, uint64_t blob_size)
    : value_type(ValueType::BLOB), blob_val(blob_ptr), size(blob_size) {}

// SQLiteResultIterator
SQLiteResultIterator::SQLiteResultIterator(sqlite3_stmt* prepared_stmt,
    std::shared_ptr<int64_t> shared_idx, std::shared_ptr<std::vector<std::string>> cols,
    int rescode)
  ...
  for (unsigned iCol = 0; iCol < cols->size(); iCol++) {
    switch ((SQLValue::ValueType)sqlite3_column_type(prepared_stmt, iCol)) {
      ...
      case SQLValue::ValueType::TEXT:
        result.push_back(
            SQLValue(std::string((const char*)sqlite3_column_text(prepared_stmt, iCol))));
        break;
      ...
      case SQLValue::ValueType::NULL_VALUE: result.push_back(SQLValue()); break;
    }
  }
}
...
bool SQLiteResultIterator::operator!=(const SQLiteResultIterator& lvalue) {
  if ((lvalue.result_code() == SQLITE_DONE || lvalue.result_code() == SQLITE_OK) &&
      (result_code() == SQLITE_DONE || result_code() == SQLITE_OK)) {
    return false;
...
SQLiteResult SQLiteDB::exec(const std::string& sql, const std::map<std::string, SQLValue>& values) {
  ...
  for (auto value : values) {
    switch (value.second.get_type()) {
      case SQLValue::ValueType::INT:
        sqlite3_bind_int64(sqlite_stmt,
            sqlite3_bind_parameter_index(sqlite_stmt, value.first.c_str()), value.second.as_int());
        break;
      case SQLValue::ValueType::DOUBLE:
        sqlite3_bind_double(sqlite_stmt,
            sqlite3_bind_parameter_index(sqlite_stmt, value.first.c_str()),
            value.second.as_double());
        break;
      case SQLValue::ValueType::TEXT: {
        auto text_data = value.second.as_text();
        sqlite3_bind_text64(sqlite_stmt,
            sqlite3_bind_parameter_index(sqlite_stmt, value.first.c_str()), text_data.data(),
            text_data.size(), SQLITE_TRANSIENT, SQLITE_UTF8);
      } break;
      case SQLValue::ValueType::BLOB: {
        auto blob_data = value.second.as_blob();
        sqlite3_bind_blob64(sqlite_stmt,
            sqlite3_bind_parameter_index(sqlite_stmt, value.first.c_str()), blob_data.data(),
            blob_data.size(), SQLITE_TRANSIENT);
      } break;
      case SQLValue::ValueType::NULL_VALUE:
        sqlite3_bind_null(
            sqlite_stmt, sqlite3_bind_parameter_index(sqlite_stmt, value.first.c_str()));
        break;
    }
  }
