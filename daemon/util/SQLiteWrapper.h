...
class SQLValue {
 ...
 public:
  ...
  SQLValue(std::array<uint8_t, array_size> blob_array)
      : SQLValue(blob_array.data(), blob_array.size()) {}
  ...
  std::array<uint8_t, array_size> as_blob() const {
    ...
    std::copy(blob_val, blob_val + std::min(size, array_size), new_array.data());
    return new_array;
  }
