...
try:
    ...
    version_string = out.stdout.decode("utf-8").strip()

if len(sys.argv) > 1 and sys.argv[1] == 'versionstring':
    version_string = version_string[1:]
elif len(sys.argv) > 1 and sys.argv[1] == 'versioninfo':
    matches = re.match('^v(\d+)\.(\d+)\.(\d+)(?:-(\d+))?', version_string)
    groups = matches.groups("0")
    version_string = groups[0] + "." + groups[1] + "." + groups[2] + "." + groups[3]
elif len(sys.argv) > 1 and sys.argv[1] == 'debian':
    matches = re.match('^v(\d+)\.(\d+)\.(\d+)(?:-(\d+)-g(\w+))?', version_string)
    groups = matches.groups("0")
    if groups[3] == "0":
