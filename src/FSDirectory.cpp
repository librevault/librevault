...
FSDirectory::FSDirectory(ptree dir_options, boost::asio::dir_monitor& monitor, io_service& ios, Signals& signals, ptree& options) :
		ios(ios), monitor(monitor), dir_options(std::move(dir_options)), options(options), signals(signals) {
