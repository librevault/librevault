...
// TrackerProvider
TrackerProvider::TrackerProvider(QByteArray node_id, PortMappingService* port_mapping, QNetworkAccessManager* nam,
                                 QObject* parent)
    : QObject(parent), node_id_(node_id), port_mapping_(port_mapping), nam_(nam) {}
TrackerProvider::~TrackerProvider() {}

QByteArray TrackerProvider::getNodeId() const { return node_id_; }
...
void TrackerProvider::addGroup(const QByteArray& groupid) {
  groups_[groupid] = std::make_unique<TrackerGroup>(groupid, getNodeId(), getExternalPort(), nam_, this);
