...
AssemblerQueue::AssemblerQueue(const FolderParams& params, MetaStorage* meta_storage, ChunkStorage* chunk_storage,
  ...
  assemble_timer_->setTimerType(Qt::VeryCoarseTimer);
...
void AssemblerQueue::addAssemble(const SignedMeta& smeta) {
  auto* worker = new AssemblerWorker(smeta, params_, meta_storage_, chunk_storage_, path_normalizer_, archive_);
