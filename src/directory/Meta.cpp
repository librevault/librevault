...
blob Meta::serialize() const {
	unicbor::object o;
	o.reset(unicbor::object::MAP);

	o.add_item("path_id");
	o.add_item(path_id());

	o.add_item("encrypted_path");
	o.add_item(encrypted_path());

	o.add_item("encrypted_path_iv");
	o.add_item(encrypted_path_iv());

	o.add_item("meta_type");
	o.add_item((uint8_t)meta_type());

	o.add_item("revision");
	o.add_item(revision());

	o.add_item("symlink_encrypted_path");
	o.add_item(symlink_encrypted_path());

	o.add_item("symlink_encrypted_path_iv");
	o.add_item(symlink_encrypted_path_iv());

	o.add_item("windows_attrib");
	o.add_item(windows_attrib());
	...
	o.add_item("max_blocksize");
	o.add_item(max_blocksize());

	o.add_item("min_blocksize");
	o.add_item(min_blocksize());

	o.add_item("strong_hash_type");
	o.add_item((uint8_t)strong_hash_type());

	o.add_item("weak_hash_type");
	o.add_item((uint8_t)weak_hash_type());
	...
	std::ostringstream os;
	unicbor::packer packer(os);
	packer.pack(o);

	std::string serialized_string = os.str();

	return blob(serialized_string.begin(), serialized_string.end());
}

void Meta::parse(const blob &serialized_data) {
	unicbor::object o;
	unicbor::unpacker unpacker(o);

	unpacker.add_bytes(serialized_data.data(), serialized_data.size());
