...
std::vector<uint8_t> convert_bitfield(const boost::dynamic_bitset<uint8_t>& bitfield) {
...
boost::dynamic_bitset<uint8_t> convert_bitfield(blob bitfield) {
	boost::dynamic_bitset<uint8_t> converted_bitfield(bitfield.size()*8);
	for(auto& b : bitfield) {
