...
pub extern "C" fn nodekey_write_new(key_path: *const c_char) {
    ...
    key_file.write_all(pem.as_slice());
}
...
pub extern "C" fn nodekey_write_new_cert(key_path: *const c_char, cert_path: *const c_char) {
    ...
    trace!("PEM certificate: {}", String::from_utf8_lossy(pem.as_slice()));
    trace!("PEM digest: {}", hex::encode(certificate.digest(MessageDigest::sha256()).unwrap()));
